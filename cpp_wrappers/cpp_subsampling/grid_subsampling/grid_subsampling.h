...
public:
	...
	// Method Update
	void update_all(const PointXYZ p, vector<float>::iterator f_begin, vector<int>::iterator l_begin, vector<int>::iterator l_ins_begin)
	{
		count += 1;
		point += p;
        ...
        i = 0;
		for(vector<int>::iterator it = l_ins_begin; it != l_ins_begin + labels.size(); ++it)
		{
		    ins_labels[i][*it] += 1;
		    i++;
		}
	...
	void update_classes(const PointXYZ p, vector<int>::iterator l_begin, vector<int>::iterator l_ins_begin)
	{
		count += 1;
		point += p;
		int i = 0;
