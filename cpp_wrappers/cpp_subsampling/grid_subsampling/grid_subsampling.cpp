...
void grid_subsampling(vector<PointXYZ>& original_points,
	...
	for (auto& p : original_points)
	{
		...
		if (use_feature && use_classes)
			data[mapIdx].update_all(p, original_features.begin() + i * fdim, original_classes.begin() + i * ldim, original_ins_classes.begin() + i * ldim);
		else if (use_feature)
			data[mapIdx].update_features(p, original_features.begin() + i * fdim);
		else if (use_classes)
			data[mapIdx].update_classes(p, original_classes.begin() + i * ldim, original_ins_classes.begin() + i * ldim);
		else
	...
	if (use_classes){
		...
		subsampled_ins_classes.reserve(data.size() * ldim);
	}
	for (auto& v : data)
	{
		...
		if (use_classes)
		{
		    for (int i = 0; i < ldim; i++){
		        ...
		        subsampled_ins_classes.push_back(max_element(v.second.ins_labels[i].begin(), v.second.ins_labels[i].end(),
		        [](const pair<int, int>&a, const pair<int, int>&b){return a.second < b.second;})->first);
		     }
		}
	}
                              ...
                              int max_p)
{
	...
	for (b = 0; b < original_batches.size(); b++)
	{
        ...
        if (original_classes.size() > 0)
        {
            ...
            b_o_ins_classes = vector<int>(original_ins_classes.begin () + sum_b * ldim,
                                      original_ins_classes.begin () + sum_b + original_batches[b] * ldim);
	    }
        ...
        if (b_s_points.size() <= max_p)
        {
            ...
            if (original_classes.size() > 0){
                ...
                subsampled_ins_classes.insert(subsampled_ins_classes.end(), b_s_ins_classes.begin(), b_s_ins_classes.end());
            }
        ...
        else
        {
            ...
            if (original_classes.size() > 0){
                ...
                subsampled_ins_classes.insert(subsampled_ins_classes.end(), b_s_ins_classes.begin(), b_s_ins_classes.begin() + max_p * ldim);
            }
